...
function cmd_create() {
  ...
  set -x
  aws lambda-snapstart create-function \
    --region us-east-2 \
    --function-name ${FUNCTION_NAME} \
    --code S3Bucket=billburke/function.zip \
    --handler ${HANDLER} \
    --runtime ${RUNTIME} \
...
function cmd_update() {
  ...
  set -x
  aws lambda-snapstart update-function-code \
    --region us-east-2 \
    --function-name ${FUNCTION_NAME} \
    --snap-start ApplyOn=PublishedVersions \
    --zip-file ${ZIP_FILE}
...
FUNCTION_NAME=Firecracker:1
HANDLER=io.quarkus.amazon.lambda.runtime.QuarkusStreamHandler::handleRequest
RUNTIME=java11
ZIP_FILE=fileb:///home/bburke/projects/quarkus/patriot1burke/code-with-quarkus/target/function.zip
...
then
  RUNTIME=provided
  ZIP_FILE=fileb:///home/bburke/projects/quarkus/patriot1burke/code-with-quarkus/target/function.zip
  FUNCTION_NAME=CodeWithQuarkusNative
